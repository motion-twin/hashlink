...
@:noCompletion
typedef GameControllerPtr = hl.Abstract<"dx_gctrl">;
		...
		for( e in a ){
			...
			if( p[1].charCodeAt(0) == 'a'.code ){
				...
				switch( p[0] ){
					...
					case "righttrigger": axis[4] = num;
					case "lefttrigger":  axis[5] = num;
					default:
				}
			}else{
				var btn : DInputButton = switch( p[1].charCodeAt(0) ){
					case 'b'.code: {num: Std.parseInt(p[1].substr(1)), mask: 0};
					case 'h'.code: {
						var ba = p[1].substr(1).split(".");
						if( ba == null ) 
							null;
						else
							{num: Std.parseInt(ba[0]), mask: Std.parseInt(ba[1])};
					}
					default: null;
				}
				var idx = switch( p[0] ){
